...
for sample_index in range(nr_samples):
    ...
    while True:
        ...
        if not increasing_trend:
            ...
            total_noisy_signal = -total_noisy_signal
        if False:
            ...
            plt.show()
        index_sensor_name += 1
        if index_sensor_name == len(sensor_names):
            break
...
np.save('./training_data/P30/GAN_pure_signals.npy', samples, allow_pickle=True)
# Save data to file for re-use
np.save('./training_data/P30/GAN_noisy_signals.npy', noisy_samples, allow_pickle=True)
