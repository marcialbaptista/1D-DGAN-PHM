...
class GANDenoiserSpecialized:
    ...
    def __init__(self, sensor_name, index_checkpoint):
        ...
        print('./models_GAN/' + sensor_name + '/model/ckpt-' + str(index_checkpoint))
        checkpoint.restore('./models_GAN/' + sensor_name + '/model/ckpt-' + str(index_checkpoint))
...
for sensor_name in sensor_names:
    print(sensor_name)
    GAN_per_sensors[sensor_name] = GANDenoiserSpecialized(sensor_name, '25')
    AE_per_sensors[sensor_name] = create_sensor_auto_encoder(sensor_name)

GAN_global = GANDenoiser('100')
...
def generate_and_save_images(unit, sensor_name, plot_chart=False):
    # C-MAPSS evaluation
...
for unit in sample_units:
    ...
    for sensor_name in sensor_names:
        ...
        noisy_orig = np.array(df.loc[df['unit_number'] == unit, sensor_name].values)
        all_noisy = np.array(df.loc[:, sensor_name].values)
        noisy_orig2 = (noisy_orig - np.mean(all_noisy)) / (np.std(all_noisy))
        noisy_orig4 = noisy_orig2
        if not increasing_trend:
            #print('Decreasing signal')
            noisy_stdev1 = (noisy_orig - np.min(all_noisy)) / (np.max(all_noisy) - np.min(all_noisy))
            noisy_stdev3 = (-noisy_stdev1 - np.min(noisy_stdev1)) / (np.max(noisy_stdev1) - np.min(noisy_stdev1))
        noisy_stdev5 = (noisy_orig - np.min(all_noisy)) / (np.max(all_noisy) - np.min(all_noisy))
        noisy = noisy_orig
        ...
        splined_reconstruction = np.array(predictions[0]).reshape((config.width * config.height,))
        reconstruction = scipy.signal.decimate(splined_reconstruction, 2)
        gan_signal_2 = spline_signal(reconstruction, len(noisy))
        ...
        all_noisy = np.array(df.loc[:, sensor_name].values)
        noisy_stdev2 = (noisy_orig - np.min(all_noisy)) / (np.max(all_noisy) - np.min(all_noisy))
        #print('Min and max of', sensor_name, np.min(all_noisy), np.max(all_noisy), np.min(all_noisy))
        noisy_vec, gan_signal_4 = generate_and_save_images(unit, sensor_name)


        noisy_splined = spline_signal(noisy_stdev5, config.height * config.width)
        ...
        GAN_per_sensor_predictions = GAN_per_sensors[sensor_name].generator(test_inputs, training=False)
        GAN_per_sensor_reconstruction = np.array(GAN_per_sensor_predictions[0]).reshape((config.height * config.width,))

        noisy_stdev2 = (noisy_orig - np.min(all_noisy)) / (np.max(all_noisy) - np.min(all_noisy))
        noisy_splined = spline_signal(noisy_stdev2, config.height * config.width)
        noisy_test_images = np.array([[noisy_splined]])
        test_inputs = noisy_test_images.reshape(noisy_test_images.shape[0],
                                                config.raw_size,
                                                config.raw_size,
                                                config.channels).astype('float32')
        # GAN global for all sensor feature
        #GAN_per_sensor_predictions = GAN_per_sensors[sensor_name].generator(test_inputs, training=False)
        #GAN_per_sensor_reconstruction = np.array(GAN_per_sensor_predictions[0]).reshape((config.height * config.width,))
        ...
        noisy_df = pd.DataFrame(data={'noisy_signal': np.array(noisy_stdev2)})
        ma_signal = noisy_df.rolling(10, min_periods=1).mean()
        ma_signal['noisy_signal'][0] = noisy_stdev2[0]
        # Median (Med) filter
        med_signal = noisy_df.rolling(10,min_periods=1, win_type='gaussian').mean(std=df.std().mean())
        med_signal['noisy_signal'][0] = noisy_stdev2[0]
        # SG filter
        window_length = 11
        sg_signal = noisy_df.apply(lambda srs: signal.savgol_filter(srs.values, window_length, 3))
        for i in range(window_length):
            sg_signal['noisy_signal'][i] = ma_signal['noisy_signal'][i]

        # auto-encoder
        predictions_AE_per_sensor = AE_per_sensors[sensor_name].predict(test_inputs)
        predictions_AE_global = AE_global.predict(test_inputs)
        gan_signal_global = list(spline_signal_back_to_original(GAN_global_reconstruction, noisy_stdev2, increasing_trend))

        gan_signal = list(spline_signal_back_to_original(GAN_per_sensor_reconstruction, noisy_stdev2, increasing_trend))
        AE_signal_global = spline_signal_back_to_original(predictions_AE_global, noisy_stdev2, increasing_trend)
        AE_signal_per_sensor = spline_signal_back_to_original(predictions_AE_per_sensor, noisy_stdev2, increasing_trend)

        df.loc[df['unit_number'] == unit, sensor_name + '_RAW'] = noisy_stdev2
        ...
        dict_names = {'_Med': 'Med', '_MA': 'MA', '_SG': 'SG', '_AEGlob': 'GDAE', '_AE': 'SDAE', '_GAN': 'SDGAN', '_GANGlob': 'GDGAN'}
        CB91_Blue = '#2CBDFE'
        CB91_Green = '#47DBCD'
        CB91_Pink = '#F3A0F2'
        CB91_Purple = '#9D2EC5'
        ...
        colors = [CB91_Blue, CB91_Green, CB91_Purple, CB91_Amber, CB91_Pink, 'red', 'black']

        i = 0
        if False or show_denoising_plots:
            #fig = plt.figure(figsize=(12, 8))
            plt.scatter(range(len(noisy_stdev2)), noisy_stdev2, color='grey', alpha=0.5)
            for model_name in ['_Med', '_MA',  '_SG']:
                ...
                mon = round(mon,2)
                plt.plot((signal_output), label='Model ' + dict_names[model_name] + ' Mon=' + str(mon), lw=2, c=colors[i])
                i += 1
...
if False:
    ...
    for sensor_name in sensor_names:
        print(sensor_name, '&', monotonicities_sensors[sensor_name],'&',
    ...
    if True:
        print('Average:', round(monotonicities_raw, 2), '&',
                  round(monotonicities_Med, 2),'&',
                  round(monotonicities_MA, 2), '&',
                  round(monotonicities_SG, 2), '&',
                  round(monotonicities_AE, 2),'&',
                  round(monotonicities_AEGlob, 2),'&',
                  round(monotonicities_GAN, 2),'&',
                  round(monotonicities_GANGlob, 2)
                  )
        print('')
    ...
    if True:
        print('Average:', round(trendabilities_Raw, 2), '&',
              round(trendabilities_Med, 2),'&',
              round(trendabilities_MA, 2), '&',
              round(trendabilities_SG, 2), '&',
              round(trendabilities_AE, 2),'&',
              round(trendabilities_AEGlob, 2),'&',
              round(trendabilities_GAN, 2),'&',
              round(trendabilities_GANGlob, 2)
              )
        print('')
    ...
    if True:
        print('Average:', round(prognosabilities_Raw, 2), '&',
              round(prognosabilities_Med, 2), '&',
              round(prognosabilities_MA, 2), '&',
              round(prognosabilities_SG, 2), '&',
              round(prognosabilities_AE, 2), '&',
              round(prognosabilities_AEGlob, 2), '&',
              round(prognosabilities_GAN, 2), '&',
              round(prognosabilities_GANGlob, 2))
        print('')
    import random
