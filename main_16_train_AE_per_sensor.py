...
sensor_names2 = ['T24', 'T30', 'T50', 'P30', 'Nf', 'Nc', 'Ps30', 'phi', 'NRf', 'NRc', 'BPR', 'htBleed', 'W31', 'W32']
...
for index_sensor_name in range(len(sensor_names)):
    ...
    #noisy_samples = np.load('./training_data/' + sensor_name + '/GAN_noisy_signals.npy', allow_pickle=True)
    #samples = np.load('./training_data/' + sensor_name + '/GAN_pure_signals.npy', allow_pickle=True)
    ...
    y_noisy_sensor_reshaped = list(itertools.chain.from_iterable(noisy_samples))
    ...
    batch_size = 100  # CHANGE 100, 10, 1000
    no_epochs = 100 # 1000
    ...
    for i in range(0, len(y_val_noisy)):
        ...
        if pure_sample[-1] < pure_sample[0]:
            pure_sample = -pure_sample - np.min(-pure_sample)
            noisy_sample = -noisy_sample - np.min(-noisy_sample)

        #noisy_sample = (noisy_sample - min_value) / (max_value - min_value)
        #pure_sample = (pure_sample - min_value) / (max_value - min_value)

        #noisy_sample = (noisy_sample - np.mean(df[sensor_name_good].values)) / (np.std(df[sensor_name_good].values))
        #pure_sample = (pure_sample - np.mean(df[sensor_name_good].values)) / (np.std(df[sensor_name_good].values))
    ...
    # or save to csv:
    hist_csv_file = './logs/AE/' + sensor_name + '/AE_logs_history.csv'
    ...
    import os
    #os.mkdir('./models_AE/'+ sensor_name)
