...
print("Number of total training units:", int(len(y_pure)/len(sensor_names)))
...
if True:
    ...
    y_pure = []
    for sensor_name in sensor_names:
        y_noisy_sensor = np.load('./training_data2/' + sensor_name + '/GAN_noisy_signals.npy', allow_pickle=True)
        y_noisy.extend(y_noisy_sensor)
        y_pure.extend(np.load('./training_data2/' + sensor_name + '/GAN_pure_signals.npy', allow_pickle=True))
...
for sensor_name in sensor_names:
    ...
    for m in range(sensor_indexes[j]):
        noisy_sample = noisy_samples[i]
        pure_sample = samples[i]
...
if False:
    ...
    for i in range(len(y_noisy)):
        ...
        min_value = np.min(df[sensor_names].min())
        max_value = np.max(df[sensor_names].max())
        noisy_sample = (noisy_sample - min_value) / (max_value - min_value)
        pure_sample = (pure_sample - min_value) / (max_value - min_value)
...
model_path = os.path.join('models_GAN2', 'global', 'model')
results_path = os.path.join('models_GAN2', 'global', 'results')
